...
function TextAnimationTrigger(Text, intervalTime, delayTime) {
  ...
  const colors = [
    { main: '#EEC05B', shades: ['#FAE073', '#FCE790', '#FADD65', '#E4C650'] },
    { main: '#E0A62F', shades: ['#F7B989', '#F9CDAA', '#DD8644', '#F39C59'] },
    { main: '#CD902D', shades: ['#EE7293', '#F191AB', '#D64D72', '#C04567'] },
    { main: '#B18341', shades: ['#B084B6', '#C19FC7', '#916198', '#82588A'] },
    { main: '#C49629', shades: ['#6382B9', '#829BC7', '#4D6CA3', '#3E5782'] },
    { main: '#CD922E', shades: ['#4DBFAD', '#73CDBF', '#27A18D', '#1F8171'] },
    { main: '#F19E00', shades: ['#7FBE90', '#98CBA6', '#68A87A', '#5E976E'] },
  ];
  ...
  const animateLetterIn = letter => {
    ...
    const rotation = -50 + Math.random() * 300;
  ...
  const addDecoration = (letter, color) => {
    setTimeout(() => {
      ...
      for (let i = 0; i < 50; i += 1) addTriangle(x0, y0, shade);
      for (let i = 0; i < 10; i += 1) addCircle(x0, y0);
    }, 150);
  };
  ...
  const addDecorationOnHover = (letter, color) => {
    setTimeout(() => {
      ...
      const shade = color.shades[Math.floor(Math.random() * 4)];
      for (let i = 0; i < 100; i += 1) addTriangle(x0, y0, shade);
  ...
  window.addEventListener('click', e => {
    for (let i = 0; i < 100; i++) {
      ...
      addTriangle(e.clientX, e.clientY, 'white');
    }
  });
