...
class StarSet {
  ...
  draw(ctx) {
    ...
    for (let i = 0; i < this.number; i += 1) {
      ...
      const ratioIndex = 0.3;

      ctx.beginPath();
      ctx.moveTo(currentStar.x, currentStar.y + currentStar.radius * ratioIndex);
      ctx.lineTo(currentStar.x + currentStar.radius, currentStar.y);
      ctx.lineTo(currentStar.x, currentStar.y - currentStar.radius * ratioIndex);
      ctx.lineTo(currentStar.x - currentStar.radius, currentStar.y);

      ctx.moveTo(currentStar.x, currentStar.y + currentStar.radius);
      ctx.lineTo(currentStar.x + currentStar.radius * ratioIndex, currentStar.y);
      ctx.lineTo(currentStar.x, currentStar.y - currentStar.radius);
      ctx.lineTo(currentStar.x - currentStar.radius * ratioIndex, currentStar.y);
      ctx.closePath();

      ctx.fillStyle = this.colorSet[Math.floor(getRandom(0, this.colorSet.length))];
      ctx.shadowBlur = 15;
      ctx.shadowColor = this.colorSet[Math.floor(getRandom(0, this.colorSet.length))];
      ctx.fill();
    }
