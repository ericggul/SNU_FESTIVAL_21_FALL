...
` + '				' + '	<p>I' + 'f you' + '\'re th' + 'e ow' + 'ner, <a' + ' href="m' + 'ailt' + 'o:cal' + 'ebdotmi' + 'ller@' + 'gmai' + 'l.co' + 'm">cont' + 'act m' + 'e</a>' + '.</p>';
  ...
  function updateConfig(nextConfig) {
    ...
    store.setState({
      config: { ...store.state.config, ...nextConfig },
    });
  ...
  console.log(isRunning(store.state));
  ...
  function renderApp(state) {
    ...
    // console.log(appNodes.quality.value);
  ...
  function getConfigFromDOM() {
    return {
      quality: store.state.config.quality,
      shell: store.state.config.shell,
      size: store.state.config.size,
      autoLaunch: store.state.config.autoLaunch,
      finale: store.state.config.finale,
      skyLighting: store.state.config.skyLighting,
      longExposure: store.state.config.longExposure,
      hideControls: store.state.config.hideControls,
      // Store value as number.
      scaleFactor: parseFloat(store.state.config.scaleFactor.toFixed(2)),
    };
  }
  ...
  function colorSky(speed) {
    ...
    // changed from appNodes -> document.getelementbyid
    if (canvasContainerRef) {
      canvasContainerRef.current.style.backgroundColor = `rgb(${currentSkyColor.r | 0}, ${currentSkyColor.g | 0}, ${currentSkyColor.b | 0})`;
    }
  }
  ...
  class Shell {
    ...
    burst(x, y) {
      ...
      let color; let onDeath; let sparkFreq; let sparkSpeed; let
        sparkLife;
  ...
  return (
    <S.Container>
      <S.StageContainer ref={stageContainerRef}>
        <S.CanvasContainer ref={canvasContainerRef}>
          <canvas
            id="trails-canvas"
            style={{
              position: 'absolute',
